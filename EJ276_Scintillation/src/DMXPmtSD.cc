...
#include "G4OpticalPhoton.hh" //For PID of optical photons only.
...
//----------------------------Only for Optical Photons-----------------------------//
    // 1. Get the particle definition from the track
  // G4Track* track = aStep->GetTrack();
  // G4ParticleDefinition* particleType = track->GetDefinition();

  // // 2. CHECK: Only proceed if the particle is an Optical Photon
...
// //----------------------------Only for Optical Photons-----------------------------//
  ...
  aPmtHit->SetPID(aStep->GetTrack()->GetDefinition()->GetPDGEncoding());//For PID
